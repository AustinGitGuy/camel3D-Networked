...
typedef		unsigned char						TTTIndex;
typedef		TTTSpaceState			TTTGame[GS_TICTACTOE_BOARD_WIDTH][GS_TICTACTOE_BOARD_HEIGHT];
...
inline TTTSpaceState TTTGetSpace(TTTGame const game, TTTIndex const xpos, TTTIndex const ypos)
{
...
inline TTTSpaceState TTTSetSpace(TTTGame game, TTTSpaceState const state, TTTIndex const xpos, TTTIndex const ypos)
{
	if (GS_TICTACTOE_VALID(xpos, ypos))
...
inline TTTIndex TTTReset(TTTGame game)
{
	TTTIndex xpos, ypos, total;
...
inline void TTTDisplay(TTTGame game){
	for (int i = 0; i < GS_TICTACTOE_BOARD_WIDTH; i++){
		for (int c = 0; c < GS_TICTACTOE_BOARD_HEIGHT; c++){
			//Iterate through the game and print out the game state.
